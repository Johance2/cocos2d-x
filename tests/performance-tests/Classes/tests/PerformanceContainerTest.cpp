...
void PerformanceContainerScene::onEnter()
{
    TestCase::onEnter();
    if (isAutoTesting()) {
        this->_startItem->setEnabled(false);
...
void PerformanceContainerScene::onExit()
{
    TestCase::onExit();
    auto director = Director::getInstance();
    auto sched = director->getScheduler();
...
void PerformanceContainerScene::updateStatus()
{
...
void PerformanceContainerScene::switchTestType(unsigned int index)
{
    this->_type = index;
...
void PerformanceContainerScene::dumpProfilerInfo(float dt)
{
    ...
    if (this->isAutoTesting()) {
        ...
        Profile::getInstance()->addTestResult(genStrVector(_typePrefix.c_str(), _testFunctions[_type].name, numStr.c_str(), nullptr),
                                              genStrVector(avgStr.c_str(), minStr.c_str(), maxStr.c_str(), nullptr));

        auto testsSize = sizeof(autoTestCounts)/sizeof(int);
